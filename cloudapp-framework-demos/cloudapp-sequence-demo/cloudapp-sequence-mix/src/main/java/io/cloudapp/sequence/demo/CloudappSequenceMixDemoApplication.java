...
public class CloudappSequenceMixDemoApplication implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        ...
        for (int i = 0 ; i < 1000; i++) {
            System.out.println("Loop generating first queue sequence id: " + snowflakeGenerator.nextId());
        }

        System.out.println("Time snowflake generator takes to generate IDs in milliseconds: " + (
                System.currentTimeMillis() - start
        ));

        start = System.currentTimeMillis();
        for (int i = 0 ; i < 1000; i++) {
            System.out.println("Loop generating first queue sequence id: " + redisGenerator.nextId());
        }

        System.out.println("Time redis generator takes to generate IDs in milliseconds: " + (
                System.currentTimeMillis() - start
        ));
    }
}
